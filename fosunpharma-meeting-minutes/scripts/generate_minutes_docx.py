...
def add_metadata(document: Document, minutes: dict[str, Any], profiles: dict[str, dict[str, Any]]) -> None:
    ...
    participants = "；".join(str(person) for person in minutes.get("participants") or ["【待补充】"])
    add_body_item(document, f"参会人员：{participants}", profiles)
    add_body_item(document, f"主持人：{metadata.get('主持人', '【待补充】')}", profiles)
